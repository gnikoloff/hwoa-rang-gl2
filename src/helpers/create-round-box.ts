...
/*
  ...
  This method is courtesy of https://twitter.com/SketchpunkLabs  
  Taken from https://sketchpunk.bitbucket.io/src/fungi_v5_5/016_round_cube_edge.html

  Few modifications from Georgi:

  - Each face has texture space from (1 / 6) * idx to (1 / 6) * (idx + 1)
    This gives me the ability to texture and color each face individually
  
  - I made sure to interleave all the vertices, uvs and normals in a single
...
const createRoundBox = ({
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  const vertexCount = geo.verts.length / 3
  const verticesNormalUvs = new Float32Array(
    geo.verts.length + geo.norm.length + geo.uv.length,
  )
  // pos + normal + uv
  const vertexStride = 3 + 3 + 2
  for (let i = 0; i < vertexCount; i++) {
    // pos
    verticesNormalUvs[i * vertexStride + 0] = geo.verts[i * 3 + 0]
    verticesNormalUvs[i * vertexStride + 1] = geo.verts[i * 3 + 1]
    verticesNormalUvs[i * vertexStride + 2] = geo.verts[i * 3 + 2]
    // normal
    verticesNormalUvs[i * vertexStride + 3] = geo.norm[i * 3 + 0]
    verticesNormalUvs[i * vertexStride + 4] = geo.norm[i * 3 + 1]
    verticesNormalUvs[i * vertexStride + 5] = geo.norm[i * 3 + 2]
    // uv
    verticesNormalUvs[i * vertexStride + 6] = geo.uv[i * 2 + 0]
    verticesNormalUvs[i * vertexStride + 7] = geo.uv[i * 2 + 1]
  }
  ...
  return {
    ...
    vertexStride,
    interleavedArray: verticesNormalUvs,
    indicesArray: new Uint16Array(geo.indices),
  }
}
...
function geo_rot_merge(geo, obj, fn_rot, idx) {
